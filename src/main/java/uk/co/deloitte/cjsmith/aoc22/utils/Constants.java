package uk.co.deloitte.cjsmith.aoc22.utils;

public class Constants {
    public static final String INPUT_DIR = "./src/main/resources/input/";

    public static final String DAY_1_DIR = INPUT_DIR + "01/";
    public static final String DAY_2_DIR = INPUT_DIR + "02/";
    public static final String DAY_3_DIR = INPUT_DIR + "03/";
    public static final String DAY_4_DIR = INPUT_DIR + "04/";
    public static final String DAY_5_DIR = INPUT_DIR + "05/";
    public static final String DAY_6_DIR = INPUT_DIR + "06/";
    public static final String DAY_7_DIR = INPUT_DIR + "07/";
    public static final String DAY_8_DIR = INPUT_DIR + "08/";
    public static final String DAY_9_DIR = INPUT_DIR + "09/";
}
